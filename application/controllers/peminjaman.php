...
class Peminjaman extends CI_Controller {
    ...
    public function cetak_peminjaman()
    {
        ...
        $this->db->join('anggota', 'anggota.anggota_id = peminjaman.anggota_id');
        
        if ($bulan) {
            $this->db->where('DATE_FORMAT(peminjaman.tanggal_pinjam, "%Y-%m")', $bulan);
        }
        
        $data['data'] = $this->db->get()->result();
        $data['bulan'] = $bulan;
        
        $this->load->view('peminjaman/cetak_peminjaman', $data);
    }
